...
class PlantDiseaseClassification(Dataset):
    ...
    CLASS_NAMES = ('apple-apple_scab', 'apple-black_rot', 'apple-cedar_apple_rust', 'apple-healthy', 
                   'blueberry-healthy', 'cherry_(including_sour)-healthy', 'cherry_(including_sour)-powdery_mildew', 
                   'corn_(maize)-cercospora_leaf_spot gray_leaf_spot', 'corn_(maize)-common_rust_', 'corn_(maize)-healthy', 
                   'corn_(maize)-northern_leaf_blight', 'grape-black_rot', 'grape-esca_(black_measles)', 'grape-healthy', 
                   'grape-leaf_blight_(isariopsis_leaf_spot)', 'orange-haunglongbing_(citrus_greening)', 'peach-bacterial_spot', 
                   'peach-healthy', 'pepper_bell-bacterial_spot', 'pepper_bell-healthy', 'potato-early_blight', 'potato-healthy', 
                   'potato-late_blight', 'raspberry-healthy', 'soybean-healthy', 'squash-powdery_mildew', 'strawberry-healthy', 
                   'strawberry-leaf_scorch', 'tomato-bacterial_spot', 'tomato-early_blight', 'tomato-healthy', 'tomato-late_blight', 
                   'tomato-leaf_mold', 'tomato-septoria_leaf_spot', 'tomato-spider_mites two-spotted_spider_mite', 'tomato-target_spot', 
    ...
    def stratified_df(cls, root: Path, val_split: float, test_split: float, random_seed: int) -> DataFrame: 
        df = DataFrame({"name": list(root.rglob("*.JPG")) + list(root.rglob("*.jpg"))})
