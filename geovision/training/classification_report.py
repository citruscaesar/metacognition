...
def classification_report_dict(confusion_matrix: Tensor, prefix:str, class_names: Optional[tuple[str, ...]] = None) -> dict[str, float]:
    confusion_matrix_ndarray = confusion_matrix.cpu().numpy()
    df = classification_report_df(confusion_matrix_ndarray, class_names)
    metrics = dict()
...
def classification_report_df(confusion_matrix: NDArray, class_names: Optional[tuple[str, ...]] = None) -> pd.DataFrame:
